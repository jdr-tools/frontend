 ...
 */
const Authentication = function authenticationFunction ($localStorage, $rootScope, $state, $timeout, $translate, Api, AccountsFactory, WebsocketChannel) {
    'ngInject'
  ...
  vm.sessionCreationSuccess = (session_response) => {
    ...
    $timeout(() => {
      AccountsFactory.own((account_response) => {
        ...
        $translate.use(account_response.account.language)
